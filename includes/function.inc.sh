...
function welcome
{
  welcome_short
  if [ "$OS" = "rocky9" ]; then
    echo "  * PHP 7.4 ~ 8.4 + Nginx + Let's Encrypt + MariaDB installer"
  elif [ "$OS" = "rocky8" ]; then
...
function options
{
  ...
  if [ ! -f "locks/scripts_php7-remi-install.sh_73" ]; then
    if [ $PHP73 = "1" ]; then
      if [ "$OS" = "rocky9" ]; then
        printf "  - Rocky Linux 9 에서는 ${GREEN}PHP 7.3${NO_COLOR} 을 지원하지 않습니다. (OpenSSL 3 호환 불가)\n"
      else
  ...
  if [ ! -f "locks/scripts_php7-remi-install.sh_72" ]; then
    if [ $PHP72 = "1" ]; then
      if [ "$OS" = "rocky9" ]; then
        printf "  - Rocky Linux 9 에서는 ${GREEN}PHP 7.2${NO_COLOR} 를 지원하지 않습니다. (OpenSSL 3 호환 불가)\n"
      else
  ...
  if [ ! -f "locks/scripts_php7-remi-install.sh_71" ]; then
    if [ $PHP71 = "1" ]; then
      if [ "$OS" = "rocky9" ]; then
        printf "  - Rocky Linux 9 에서는 ${GREEN}PHP 7.1${NO_COLOR} 을 지원하지 않습니다. (OpenSSL 3 호환 불가)\n"
      else
  ...
  if [ ! -f "locks/scripts_php7-remi-install.sh_70" ]; then
    if [ $PHP70 = "1" ]; then
      if [ "$OS" = "rocky9" ]; then
        printf "  - Rocky Linux 9 에서는 ${GREEN}PHP 7.0${NO_COLOR} 을 지원하지 않습니다. (OpenSSL 3 호환 불가)\n"
      else
  ...
  if [ ! -f "locks/scripts_php5-remi-install.sh_56" ]; then
    if [ $PHP56 = "1" ]; then
      if [ "$OS" = "rocky9" ]; then
        printf "  - Rocky Linux 9 에서는 ${GREEN}PHP 5.6${NO_COLOR} 을 지원하지 않습니다. (OpenSSL 3 호환 불가)\n"
      else
  ...
  if [ ! -f "locks/scripts_php5-remi-install.sh_55" ]; then
    if [ $PHP55 = "1" ]; then
      if [[ "$OS" = "rocky8" || "$OS" = "rocky9" ]]; then
        printf "  - Rocky Linux 8/9 에서는 ${GREEN}PHP 5.5${NO_COLOR} 를 지원하지 않습니다.\n"
      else
  ...
  if [ ! -f "locks/scripts_php5-remi-install.sh_54" ]; then
    if [ $PHP54 = "1" ]; then
      if [[ "$OS" = "rocky8" || "$OS" = "rocky9" ]]; then
        printf "  - Rocky Linux 8/9 에서는 ${GREEN}PHP 5.4${NO_COLOR} 를 지원하지 않습니다.\n"
      else
  ...
  if [[ ! -f "locks/scripts_centos7-php53-install.sh" && ! -f "locks/scripts_centos6-php53-install.sh" ]]; then
    if [ $PHP53 = "1" ]; then
      if [[ "$OS" = "rocky8" || "$OS" = "rocky9" ]]; then
        printf "  - Rocky Linux 8/9 에서는 ${GREEN}PHP 5.3${NO_COLOR} 을 지원하지 않습니다.\n"
      else
        if [ ${OS} = "centos7" ]; then
